...
def deduplicate_and_format_sources(
    ...
    # Format output
    formatted_text_list = ["Content from sources:\n"]
    for i, source in enumerate(unique_sources.values(), 1):
        formatted_text_list.append(f"{'='*80}\n")  # Clear section separator
        formatted_text_list.append(f"Source: {source['title']}\n")
        formatted_text_list.append(f"{'-'*80}\n")  # Subsection separator
        formatted_text_list.append(f"URL: {source['url']}\n===\n")
        formatted_text_list.append(f"Most relevant content from source: {source['content']}\n===\n")
        if include_raw_content:
            ...
            formatted_text_list.append(f"Full source content limited to {max_tokens_per_source} tokens: {raw_content}\n\n")
        formatted_text_list.append(f"{'='*80}\n\n") # End section separator
                
    return "".join(formatted_text_list).strip()
